...
for (let DishServed in lunchBox) {
	if (DishServed.includes("side")){
		console.log(`I found ${lunchBox[DishServed]}!`);
	}
};
...
function printItemsAtIndex(index) {
	...
	for (let indexAt in pantry){
		if (pantry[indexAt][index] !== undefined){
			console.log(`Items at index:\t${pantry[indexAt][index]}`);
		}
	}
};
printItemsAtIndex(2);
...
function packageTypes(type) {
	const items = pantry[type];
	console.log(`Items that are ${type}: ${items}`);
}
